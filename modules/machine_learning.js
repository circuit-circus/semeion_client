...
function parseData(data) {
	...
	for(var i = 0; i < data.length; i++) {
		let newObj = {
			"output" : {
				"idleSpeed": data[i].idleSpeed,
				"minIdleBri": data[i].minIdleBri,
				"maxIdleBri": data[i].maxIdleBri,
				"minIdleCol": data[i].minIdleCol,
				"maxIdleCol": data[i].maxIdleCol,
				"deUnSpeed": data[i].deUnSpeed,
				"minDeUnBri": data[i].minDeUnBri,
				"maxDeUnBri": data[i].maxDeUnBri,
				"minDeUnCol": data[i].minDeUnCol,
				"maxDeUnCol": data[i].maxDeUnCol,
				"dePlSpeed": data[i].dePlSpeed,
				"minDePlBri": data[i].minDePlBri,
				"maxDePlBri": data[i].maxDePlBri,
				"minDePlCol": data[i].minDePlCol,
				"maxDePlCol": data[i].maxDePlCol,
				"acUnSpeed": data[i].acUnSpeed,
				"minAcUnBri": data[i].minAcUnBri,
				"maxAcUnBri": data[i].maxAcUnBri,
				"minAcUnCol": data[i].minAcUnCol,
				"maxAcUnCol": data[i].maxAcUnCol,
				"acPlSpeed": data[i].acPlSpeed,
				"minAcPlBri": data[i].minAcPlBri,
				"maxAcPlBri": data[i].maxAcPlBri,
				"minAcPlCol": data[i].minAcPlCol,
				"maxAcPlCol": data[i].maxAcPlCol
			},
			"input" : {
				"time" : data[i].time
			}
